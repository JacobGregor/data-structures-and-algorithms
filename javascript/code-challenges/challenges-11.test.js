...
function transformToLis(obj){
  let newArr = [];
  Object.entries(obj).forEach((value) => {value[0] === 'name' ? newArr.push(`<li>name: ${value[1]}</li>`) : newArr.push(`<li>age: ${value[1]}</li>`)
  });
  return newArr;
}
...
const count = (target, input) => {
  let counter = 0;
  input.flat().filter( value => value === target ? counter++ : counter);
  return counter;
};
...
const totalSum = (input) => input.flat().reduce((x,y)=> x + y,0);
...
},
{
  ...
  let findMaleAndFemale = (data) => {
    for(let i = 0; i <= data.length -1; i++){
      for(let j = 0; j <= data[i].length -1; i++){
        if(j.gender === 'female' || 'male'){
          return `${j.name} + and `
        }
      }
    }
