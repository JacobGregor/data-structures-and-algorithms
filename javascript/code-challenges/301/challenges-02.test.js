...
const raisedToTheThird = (arr) => {
  const powerArray = [];
  arr.forEach( number => {
    powerArray.push(Math.pow(number,3));
  });
  return powerArray;
};
...
const forLoopTwoToThe = (arr) => {
  let squaredArr = [];
  arr.forEach(number => {
    squaredArr.push(Math.pow(2,number));
  });
  return squaredArr;
};
...
const forEachTwoToThe = (arr) => {
  let secondSquaredArr = [];
  arr.forEach( number => {
    secondSquaredArr.push(Math.pow(2,number));
  });
  return secondSquaredArr;
};
...
const charCode = (arr) => arr.map(x => x.charCodeAt(0));
...
const evenOdd = (arr) => arr.map(x => x%2 === 0 ? 'even' : x%2 === 1 ? 'odd' : 'N/A');
...
const extractAbilities = (arr) => arr.map( x => x.ability.name);
...
const extractStats = (arr) => arr.map(attribute => (
  {
    name: attribute.stat.name,
    total: (attribute.effort + attribute.baseStat)
  }
));
