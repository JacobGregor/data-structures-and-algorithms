...
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
const maxInArray = (arr) => arr.reduce((previous,current) => {return previous<current ? previous = current : previous;},0);
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const updateNumbers = (obj) => Object.entries(obj).map( data => `${data[0]}: ${data[1]}`);
...
const hasChildrenValues = (arr, character) => {
  let children = false;
  arr.forEach( name =>{if (name.name === character && name.children){
    children = Object.values(name.children).length > 0;
  } 
});
return children
};
