...
// arr.map(value => value.substring(0,1)); returns the [0] and stops after the first return. -> Could have also used this... 
...
const findHappiness = (arr) => arr.filter(value => value.includes(':)') ? arr : console.log('nope'));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
const onlyOddChars = (str) => {
  const newArr = [];
  str.split('').reduce((prev,curr,i) => i % 2 === 1 ? newArr.push(curr) : console.log('get outta here'),[]);
  return newArr.join('');
};
...
const allHappy = (arr) => {
  var check = true;
  arr.forEach(value => {
    !value.includes(':)') ? check = false : console.log('nope');
 });
 return check;
};
